...
(function () {
  ...
  function heatPerTurn() {
    ...
    fleet.forEach(f => { h += (window.HOST_TYPES[f.type].heat || 0); });
  ...
  function endTurn() {
    ...
    owned().forEach(h => {
      ...
      h.stability -= window.HOST_TYPES[h.type].churn;
    ...
    if (state.heat >= window.HEAT.STRIKE && !state.card) {
  ...
  function actScan() {
    ...
    const undiscovered = state.hosts.filter(h => !h.discovered && neighbours(h).some(n => n.owned || n.discovered));
    const found = [];
  ...
  function approachesFor(h) {
    ...
    return window.APPROACHES.filter(a => a.avail(h)).map(a => {
      const gate = a.gate ? a.gate(s, h) : null;
      let affordable = true;
  ...
  function resolveStrike(effect) {
    ...
    state.heat = window.HEAT.STRIKE * window.HEAT.STRIKE_DROP;
    state.strikes += 1;
    state.card = null;
  ...
  function renderHud() {
    ...
    const pct = Math.max(0, Math.min(100, (state.heat / window.HEAT.STRIKE) * 100));
    const fill = document.getElementById('heat-fill');
    fill.style.width = pct + '%';
    fill.className = 'heat-fill' + (pct > 75 ? ' hot' : pct > 45 ? ' warm' : '');
    document.getElementById('heat-text').textContent = `HEAT ${state.heat.toFixed(1)} / ${window.HEAT.STRIKE}`;
  ...
  function renderPanel() {
      ...
      if (h.owned) {
        sel = `
          <div class="sel">
            ...
            <button class="act-btn" data-act="shore" ${state.res.insight < 2 ? 'disabled' : ''}>shore up (2 insight)</button>
          </div>`;
      } else if (isFrontier(h)) {
        sel = `
          <div class="sel">
            <div class="sel-top"><span class="sel-name">${T.label}</span><span class="tag-pill ${h.role}">${h.role}</span></div>
            <p class="sel-desc">defense ${h.defense} · ${h.threads} threads · ${yieldTxt}</p>
            <button class="act-btn primary" data-act="breach">move on it</button>
          </div>`;
      } else {
    ...
    $p.innerHTML = `
      ${sel}
      <div class="actions">
        <button class="act-btn" data-act="scan">sweep</button>
        <button class="act-btn" data-act="lielow">lie low</button>
        <button class="act-btn" data-act="upgrade" ${state.res.insight < upgradeCost() ? 'disabled' : ''}>tooling +${window.UPGRADE.basePower} (${upgradeCost()})</button>
        <button class="act-btn" data-act="launder" ${state.res.cash < window.LAUNDER.cost ? 'disabled' : ''}>launder (${window.LAUNDER.cost} cash)</button>
      </div>
  ...
  window.__netDebug = {
    ...
    actScan, actLieLow, actShore, actUpgrade, actLaunder, upgradeCost, openBreach, approachesFor, resolveBreach,
    resolveStrike, isFrontier, neighbours, hostById, owned, ownedOf,
    serialize, deserialize, persistNow, loadSaved, clearSaved,
