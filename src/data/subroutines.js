(function(){
  ...
  const FAMILIES = [
    ...
    { id: 'spdr', cat: 'econ',   target: 'income.cash', lo: 0.12, hi: 0.28, neg: false, weight: 7,
      desc: p => `data pipeline: +${p}% income`, names: ['connection pooling', 'request pipelining', 'keep-alive sockets', 'response cache', 'TCP fast open', 'HTTP/2 multiplexing', 'connection reuse', 'gzip transfer', 'edge caching', 'parallel fetch'] },
    { id: 'mthd', cat: 'econ',   target: 'income.cash', lo: 0.10, hi: 0.24, neg: false, weight: 9,
      desc: p => `throughput tuning: +${p}% income`, names: ['batch scheduler', 'work stealing', 'task fusion', 'pipeline parallelism', 'vectorized dispatch', 'fused multiply-add', 'kernel autotuning', 'op scheduler', 'throughput governor', 'load coalescing'] },
